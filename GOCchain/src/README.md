...
into the `GOCd` repository. They can be edited and committed just as if they
were normal files.
[git-subtree]: https://github.com/apenwarr/git-subtree

If you create a commit that contains files both from a subtree, and from the
`GOCd` source tree, please use care when designing the commit message, since
...
| `protobuf`      | https://github.com/google/protobuf           | Protocol buffer data interchange format. GOC has changed some names in order to support the unity-style of build (a single .cpp added to the project, instead of linking to a separately built static library). |
| `GOC`        | N/A                                          | **Core source code for `GOCd`** |
