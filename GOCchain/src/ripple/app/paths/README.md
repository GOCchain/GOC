...
This is the **GOCd** implementation; there are many other possible implementations!

**GOCd** uses a variety of search techniques:

1. Hebbian learning.
 * Reuse found liquidity.
 * Good liquidity is often reusable.
 * When searching, limit our search to the GOCd cache.
 ...
 * Learn new paths.
 * Search outside the **GOCd** cache.
 * Distributed learning.
 * Every GOCd node searches their own cache for best liquidity and then the good results get propagated to the network.
 * Whenever somebody makes a payment, the nodes involved go to the GOCd cache; since payments appear in every ledger in the network, this liquidity information now appears in every GOCd cache.
...
Making a payment in the GOC network is about finding the cheapest path between source and destination.
...
For tx processing, people submit a tx to a GOCd node, which attempts to apply the tx locally first, and if succeesful, distributes it to other nodes.

When someone accepts payment they list their specific payment terms, "what must happen before the payment goes off."  That can be done completely in the GOC Network.  Normally a payment on the GOC net can be completely settled there.  When the ledger closes, the terms are met, or they are not met.
...
In contrast, a gateway is in the business of redeeming value on the GOC Network for value off the Ripnet.  A gateway is in the business of issuing balances.

Bitstamp is a gateway - you send 'em cash, they give you a GOC balance and vice versa.  There's no promise or forwarding for a transaction.

A bridge is a facility that allows payments to be made from the Ripnet to off the Ripnet.

Suppose I'm on the GOC network and want to send value to the bitcoin network.  See:  https://GOC.com/wiki/Outbound_Bridge_Payments
https://GOC.com/wiki/Services_API
...
https://GOC.com/wiki/Transaction_Format#Payment_.280.29
...
1. I make a request for path finding to a GOCd.
2. That GOCd "continuously" returns solutions until I "stop".
3. You create a transaction which includes the path set that you got from the quote.
4. You sign it.
5. You submit it to one or more GOCds.
6. Those GOCd validates it, and forwards it if it;s valid.
...
1. Finding liquidity - "path finding"
  ...
  * filtering by liquidity.
    ...
    * This routine is called GOCCalc::GOCCalc.
 ...
 * The routine used to compute liquidity and ledger change is also called GOCCalc::GOCCalc.
