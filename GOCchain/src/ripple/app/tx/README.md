...
In order to enhance the flexibility of GOC and provide support for enhanced
...
Some technical details, including tables indicating some of the GOC
commands and ledger entries to be used for implementing multi-signature, are
currently listed on the [wiki](https://GOC.com/wiki/Multisign) but will
...
transaction is to be signed outside the GOC network and only submitted
...
some messaging mechanism outside the GOC network to disseminate the proposed
...
out and discussed on the [wiki](https://GOC.com/wiki/Multisign). This
...
Currently transactions on the GOC network require the use of sequence
