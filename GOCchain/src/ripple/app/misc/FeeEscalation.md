...
GOCd's fee mechanism consists of several interrelated processes:
...
more (or less) work for GOCd to process. Those transactions may have
...
behavior of the GOC network. Note that this is not necessarily the
...
automatically as the GOC network's performance improves, allowing
...
not make sense if GOCd is not synced to the network.
...
### [`server_info`](https://GOC.com/build/GOCd-apis/#server-info) command
...
### [`server_state`](https://GOC.com/build/GOCd-apis/#server-state) command
