...
GOCd_exe=/opt/GOC/bin/GOCd
conf_file=/etc/opt/GOC/GOCd.cfg
...
tmp_loc=$(mktemp -d --tmpdir GOC_info.XXXX)
cd /tmp
chmod 751 GOC_info.*
cd ~
echo ${tmp_loc}

cleaned_conf=${tmp_loc}/cleaned_GOCd_cfg.txt
...
echo "Use the following command on your local machine to download from your GOCd instance: scp <remote_GOCd_username>@<remote_host>:${tmp_loc}/info-package.tar.gz <path/to/local_machine/directory>"| tee /dev/fd/3
