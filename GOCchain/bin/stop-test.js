...
if (process.argv[2] == null) {
  [
   'Usage: ',
   '',
   '  `node bin/stop-test.js i,j [GOCd_path] [GOCd_conf]`',
   '',
   '  Launch GOCd and stop it after n seconds for all n in [i, j}',
   '  For all even values of n launch GOCd with `--fg`',
   '  For values of n where n % 3 == 0 launch GOCd with `--fg`\n',
   'Examples: ',
   '',
   '  $ node bin/stop-test.js 5,10',
   ('  $ node bin/stop-test.js 1,4 ' +
      'build/clang.debug/GOCd $HOME/.confs/GOCd.cfg')
   ]
  ...
  var GOCdPath = process.argv[3] || 'build/GOCd'
  var GOCdConf = process.argv[4] || 'GOCd.cfg'
}
...
function start(args) {
    return child.spawn(GOCdPath, args, options);
}
function stop(GOCd) { child.execFile(GOCdPath, stop_args, options)}
...
(function oneTest(n /* seconds */, results) {
  ...
  console.log("\nLaunching `%s` with `%s` for %d seconds",
                GOCdPath, JSON.stringify(args), n);

  GOCd = start(args);
  console.log("GOCd pid: %d", GOCd.pid);
  ...
  secs_l8r(n, function(){
    console.log("Stopping GOCd after %d seconds", n);
