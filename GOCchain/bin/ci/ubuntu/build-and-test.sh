...
# Ensure APP defaults to GOCd if it's not set.
: ${APP:=GOCd}
echo "using APP: $APP"
...
if [[ ${TARGET} == "docs" ]]; then
  ...
  if [[ ${BUILD_BOTH:-} == true ]]; then
    if [[ ${TARGET} == *.unity ]]; then
      cmake --build . --target GOCd_classic -- $BUILDARGS
    else
      cmake --build . --target GOCd_unity -- $BUILDARGS
    fi
  fi
fi
popd
...
# in GOCd that we want to run
declare -a manual_tests=(
  "beast.chrono.abstract_clock"
  "beast.unit_test.print"
  "GOC.NodeStore.Timing"
  "GOC.app.Flow_manual"
  "GOC.app.NoGOCCheckLimits"
  "GOC.app.PayStrandAllPairs"
  "GOC.consensus.ByzantineFailureSim"
  "GOC.consensus.DistributedValidators"
  "GOC.consensus.ScaleFreeSim"
  "GOC.GOC_data.digest"
  "GOC.tx.CrossingLimits"
  "GOC.tx.FindOversizeCross"
  "GOC.tx.Offer_manual"
  "GOC.tx.OversizeMeta"
  "GOC.tx.PlumpBook"
)
...
if [[ ${APP} == "GOCd" ]]; then
  ...
  # Only report on src/GOC files
  export LCOV_FILES="*/src/GOC/*"
