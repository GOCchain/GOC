...
[GOC Consensus Algorithm](https://GOC.com/files/GOC/consensus/whitepaper.pdf)
as implemented in [GOCd](https://github.com/GOC/GOCd), but
focuses on its utility as a generic consensus algorithm independent of the
detailed mechanics of the GOC Consensus Ledger. Most notably, the algorithm
...
state.  For the GOC network, the shared state is the current ledger--account
...
prior ledger.  For the GOC network, transactions include payments,
...
The GOC network is a decentralized and **trust-full** network.  Anyone is free
...
Leveraging this network of trust, the GOC algorithm has two main components.
...
implementation in `GOCd`.  The document **does not** discuss correctness,
fault-tolerance or liveness properties of the algorithms or the full details of
how they integrate within `GOCd` to support the GOC Consensus Ledger.
