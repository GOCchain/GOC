## Heap profiling of GOCd with jemalloc
...
jemalloc is acquired separately from GOCd, and is not affiliated
with GOC Labs. If you compile and install jemalloc from the
...
profiling has been tested with GOCd on a Linux platform. It should
work on platforms on which both GOCd and jemalloc are available.

To link GOCd with jemalloc, the argument
...
Build GOCd with jemalloc library under /usr/local/lib and
header under /usr/local/include:

    $ scons profile-jemalloc=/usr/local

Build GOCd using clang with the jemalloc library under /opt/local/lib
