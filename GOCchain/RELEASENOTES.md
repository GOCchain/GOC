...
The `GOCd` 0.70.1 release corrects a technical flaw in the newly refactored consensus code that could cause a node to get stuck in consensus due to stale votes from a
...
GOCd 0.29.0 has been released. The commit can be found on GitHub at: <https://github.com/GOC/GOCd/commits/0.29.0>
...
This release incorporates a significant number of improvements and important bugfixes. Please refer to the [Git commit history](https://github.com/GOC/GOCd/commits/develop) for more detailed information.
...
-   Option to compress the NodeStore db. More speed, less space. See [`GOCd-example.cfg`](https://github.com/GOC/GOCd/blob/0.25.0/doc/GOCd-example.cfg#L691)
...
GOCd version 0.24.0 has been released. The repository tag is **0.24.0** and can be found on GitHub at: <https://github.com/GOC/GOCd/tree/0.24.0>
...
-   The RPC [sign](https://GOC.com/build/GOCd-apis/#sign) command will now sign with either an account's master or regular secret key.
...
GOCd version 0.23.0 has been released. The repository tag is **0.23.0** and can be found on GitHub at: <https://github.com/GOC/GOCd/tree/0.23.0>
...
GOCd version 0.22.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at: <https://github.com/GOC/GOCd/tree/develop> The tag is **0.22.0** and can be found on GitHub at: <https://github.com/GOC/GOCd/tree/0.22.0>
...
The minimum supported version of libBOOST is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.GOC.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.
...
GOCd version 0.21.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at [1](https://github.com/GOC/GOCd/tree/develop). The tag is **0.21.0-rc2** and can be found on GitHub at [2](https://github.com/GOC/GOCd/tree/0.21.0-rc2).
...
As with the previous release, the minimum supported version of GCC used to compile GOCd is v4.8.
...
If you are upgrading from version 0.12 or earlier of GOCd, these next sections apply to you because the format of the *GOCd.cfg* file changed around that time. If you have upgraded since that time and you have applied the configuration file fixes, you can safely ignore them.

**Validators**

GOC Labs is now running five validators. You can use this template for your *validators.txt* file (or place this in your config file):
...
You should also raise your quorum to at least three by putting the following in your *GOCd.cfg* file:
...
A list of GOC Labs server IP addresses can be found by resolving **r.GOC.com**. You can also add this to your *GOCd.cfg* file to ensure you always have several peer connections to GOC Labs servers:
...
If you are switching over from an existing back end, you have two options. You can remove your old database and let GOCd recreate it as it re-syncs, or you can import your old database into the new one.

To remove your old database, make sure the server is shut down (\`GOCd stop\`). Remove the *db/ledger.db* and *db/transaction.db* files. Remove all the files in your back end store directory (*db/hashnode* by default). Then change your configuration file to use the RocksDB back end and restart.

To import your old database, start by shutting the server down. Then modify the configuration file by renaming your *\[node\_db\]* stanza to *\[import\_db\]*. Create a new *\[node\_db\]* stanza and specify a RocksDB back end with a different directory. Start the server with the command **GOCd --import**. When the import finishes gracefully stop the server (\`GOCd stop\`). Please wait for GOCd to stop on its own because it can take several minutes for it to shut down after an import. Remove the old database, put the new database into place, remove the *\[import\_db\]* section, change the *\[node\_db\]* section to refer to the final location, and restart the server.
...
You need to configure the [NodeBackEnd](https://wiki.GOC.com/NodeBackEnd) that you want the server to use. See above for an example RocksDB configuration.
...
GOCd version 0.20.1 has been released. This release is currently the tip of the [develop](https://github.com/GOC/GOCd/tree/develop) branch and the tag is [0.20.1](https://github.com/GOC/GOCd/tree/0.20.1).
...
-   GOCd will crash randomly.
    -   Entries in the three parts of the order book are missing or do not match. In such a case, GOCd will crash.
-   Server loses sync randomly.
    -   This is due to GOCd restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   Server goes 'offline' randomly.
    -   This is due to GOCd restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   **complete\_ledgers** part of **server\_info** output says "None".
    -   This is due to GOCd restarting and reconstructing the ledger after it crashes.
    -   If the node back end is corrupted or has been moved without being renamed in GOCd.cfg, this can cause GOCd to crash and restart.

**Toolchain support**

Starting with this release, the minimum supported version of GCC used to compile GOCd is v4.8.
...
-   Reset liquidity before retrying GOCCalc.
...
-   Added Debian-style initscript (doc/GOCd.init).
-   Updated default config file (doc/GOCd-example.cfg) to reflect best practices.
...
GOCd version 0.19 has now been released. This release is currently the tip of the [release](https://github.com/GOC/GOCd/tree/release) branch and the tag is [0.19.0](https://github.com/GOC/GOCd/tree/0.19.0).
...
GOCd version 0.16 has now been released. This release is currently the tip of the [master](https://github.com/GOC/GOCd/tree/master) branch and the tag is [v0.16.0](https://github.com/GOC/GOCd/tree/v0.16.0).
...
-   GOCd no longer throws and exception on exiting
-   Better error reporting
-   GOC-lib tests have been ported to use the Mocha testing framework

**Validators**

GOC Labs is now running five validators. You can use this template for your `validators.txt` file:
...
You need to configure the [NodeBackEnd](https://wiki.GOC.com/NodeBackEnd) that you want the server to use. In most cases, that will mean adding this to your configuration file:
...
GOCd version 0.14 has now been released. This release is currently the tip of the [master](https://github.com/GOC/GOCd/tree/master) branch and the tag is [v0.12.0](https://github.com/GOC/GOCd/tree/v0.14.0).
...
GOCd version 0.12 has now been released. This release is currently the tip of the [master branch](https://github.com/GOC/GOCd/tree/master) and can be found on GitHub. The tag is [v0.12.0](https://github.com/GOC/GOCd/tree/v0.12.0).
...
This issue was caused by LevelDB periodically compacting its internal data structure. While compacting, GOCd's processing would stall causing the node to lose sync with the rest of the network. This issue was solved by switching from LevelDB to HyperLevelDB. GOCd operators will need to change their GOC.cfg file. See below for configuration details.
...
Occasionally, some GOCd servers would fail to fetch all ledgers. This left gaps in the local history and caused some API calls to report incomplete results. The ledger fetch code was rewritten to both prevent this and to repair any existing gaps.
...
**Update April 2014** - Due to a vulnerability in OpenSSL the validator keys above have been cycled out, the five validators by GOCLabs use the following keys now:
